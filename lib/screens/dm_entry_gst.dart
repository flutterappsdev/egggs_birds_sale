...
import '../widgets/DmItem_gst.dart';
import '../network/network.dart';

class DmEntryGst extends StatefulWidget {
  static const id = 'DmEntryGst';
  @override
  _DmEntryGstState createState() => _DmEntryGstState();
}

class _DmEntryGstState extends State<DmEntryGst> {
  ...
  final _taxper = TextEditingController();
  final _taxamount = TextEditingController();
  DateTime selectedDate = DateTime.now();

  String _chosenValue= "GST";
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
     // backgroundColor: Colors.white70,
      key: ScafoldKey,
      appBar: AppBar(
        title: Text(' GST DM : ' + dmsum.toString()),
      ),
                    ...
                    onSubmitted: (rate){
                     // print(rate);
                      setState(() {
                        ...
                        if (_radioValue1==0) {
                          amount =
                              double.parse(_qtyNos.text) *  double.parse(rate);
                          _Amount.text = amount.toStringAsFixed(0);
                         // print('nos');
                        } else {
                          amount =
                              double.parse(_qtyKg.text) * double.parse(rate);
                          _Amount.text = amount.toStringAsFixed(0);
                         // print('kgs');
                        }
                       ...
                       // print(amount);
                        setState(() {
                          dmsum = dmsum+double.parse(_Amount.text);
                        });
                      });
                    },
                  ),
              ...
              children: [
                ...
                Flexible(
                  child: TextField(
                    ...
                    style: TextStyle(color: Colors.black, fontSize: 18),
                    decoration:
                    kTextFieldDecoration.copyWith(labelText: 'Tax %'),
                    onSubmitted: (taxper) {
                      _taxamount.text = (double.parse(_Amount.text) * (double
                          .parse(taxper) / 100)).toStringAsFixed(0);
                      setState(() {
                        dmsum = dmsum+double.parse(_taxamount.text);
                      });
                    }  
                  ),
                ),
                ...
                Flexible(
                  child: TextField(
                    ...
                    style: TextStyle(color: Colors.black, fontSize: 18),
                    decoration:
                    kTextFieldDecoration.copyWith(labelText: 'Tax Amount'),
                  ),
                ),
              ],
            ...
            RoundButton(
              onPressed: () {
                ...
                dmitem.additems(
                    ...
                    _hsnCode.text,
                    _chosenValue,
                    double.parse(_taxper.text),
                    double.parse(_taxamount.text));
            ...
            Container(
              ...
              child: ListView.builder(
                ...
                itemBuilder: (ctx, i) => DmItemsGst(
                    ...
                    dmitem.items[i].taxType,
                    dmitem.items[i].taxPer,
                    dmitem.items[i].taxAmount),
              ),
            ),
            SizedBox(
              height: 8,
           ...
           : RoundButton(
              ...
              onPressed: () async {
                ...
                try {
                  ...
                  url+="TcsPer=${_tcsper.text}&TcsAmt=${_tcsamount.text}&isGST=1&ShedDetails=$_shedDetails&isDM=$_radioValue2";
                 // print(url);
                  NetworkHelper networkHelper = NetworkHelper(url);
                  var data = await networkHelper.getData();
                //  print(data);
                ...
                try {
                  for (var d in dmitem.items) {
                    String url_det =
                        '$Url/InsertDMPoultry_gst?Product=${d.prod}&QtyNos=${d.qtynos}&QtyKg=${d.qtykg}&';
                    url_det +=
                    "Amount=${d.amount.toStringAsFixed(0)}&HsnCode=${_hsnCode.text}&DMNo=$_dMNumber&Rate=${d.rate}&";
                    url_det += "AreaCode=$AreaCode&TaxType=${d.taxType}&TaxPer=${d.taxPer}&TaxValue=${d.taxAmount}";
                   // print(url_det);
